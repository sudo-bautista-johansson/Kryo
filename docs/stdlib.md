...
- **Soporta formateo con placeholders `{}`**
...
- `Vec::new()`: Create new empty vector
- `BoundedVec::new(capacity)`: Create bounded vector with capacity
...
- `Stack::alloc(value)`: Allocate on stack (default domain)
- `Heap::alloc(value)`: Allocate on heap
- `GPU::alloc(value)`: Allocate on GPU
- `Shared::alloc(value)`: Allocate in shared memory
- `Distributed::alloc(value)`: Allocate in distributed memory
- `Persistent::map(filename, value)`: Map file to persistent memory
...
println("Stack allocation: {}", stack_alloc);

// Heap allocation
let heap_alloc = Heap::alloc(valor);
println("Heap allocation: {}", heap_alloc);

// GPU allocation
let gpu_alloc = GPU::alloc(valor);
println("GPU allocation: {}", gpu_alloc);

// Shared memory
let shared_alloc = Shared::alloc(valor);
println("Shared allocation: {}", shared_alloc);

// Distributed memory
let dist_alloc = Distributed::alloc(valor);
println("Distributed allocation: {}", dist_alloc);
```
