...
Output functions with support for formatted placeholders.
...
- Unsupported placeholders remain as-is
...
Create a bounded vector with specified capacity.
...
Allocate value on stack (default domain).
...
Allocate value in shared memory.
...
Allocate value in distributed memory.
...
```kryo
Persistent::unmap(persistent);
```

#### Domain::transfer

Transfer value between memory domains.

```kryo
Domain::transfer(value);
```

#### Domain::copy

Copy value between memory domains.
